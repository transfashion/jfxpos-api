...
export const PosSyncController = {
  ...
  async prepareSalespersonSync(req, res) {
    ...
    try {
      const { possync_id, rowcount, blockcount } = await PosSyncRepository.createSalespersonSync({
        posdevice_id,
        client_timestamp: clientTime,
        datatimestamp: dataTime
      });
  ...
  async getSalespersonSync(req, res) {
    ...
    try {
      ...
      const salespersons = await PosSyncRepository.getSalespersonsByBlock(possync_id, synblock);
