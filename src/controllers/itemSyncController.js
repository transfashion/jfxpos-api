...
import { ItemSyncRepository } from '../repositories/itemSyncRepository.js';

export const ItemSyncController = {
  /**
  ...
  async prepareItemSync(req, res) {
    ...
    try {
      ...
      const { possync_id, rowcount, blockcount } = await ItemSyncRepository.createItemSync({
        posdevice_id,
  ...
  async getItemSync(req, res) {
    ...
    try {
      ...
      const items = await ItemSyncRepository.getItemsByBlock(possync_id, synblock);
